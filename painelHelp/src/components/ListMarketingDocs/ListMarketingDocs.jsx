...
const ListMarketingDocs = () => {
  ...
  const handlePrint = (selectedDoc) => {
    const printContent = `
      ...
      <div class="print-container">
        <div style="display: flex;justify-content: space-between;border-bottom: solid; padding-bottom: 10px;">
          <img src="https://media.licdn.com/dms/image/v2/C4D0BAQHZAAtje7VwKg/company-logo_200_200/company-logo_200_200/0/1630572500619/rede_drogalira_logo?e=2147483647&v=beta&t=crf7UHQ8hcFvHNLgJ99iR11P-Hoe_05CfeHdAR7TYus" alt="Logo Rede Drogalira" style="width: 80px;" />       
            <p style="font-size: x-large;">${selectedDoc.titulo}</p>    
            <p style="font-size: x-large;">Segunda Via</p>   
          <h2>${selectedDoc.idDocumento}</h2> 
        </div>
        
        <div style="display: flex; justify-content: space-between;">
          <div style="display: flex;">
            <p style="font-weight: bold;">Tipo de Documento:</p>
            <p>${selectedDoc.tipoDocumento}</p>
          </div>
          <div>
      ...
      <div>
        ...
        <ol>
          ${selectedDoc.documento.map(doc => `<li style="margin-bottom: 4px;">${doc}</li>`).join('')}
        </ol>
        </div>
      ...
      <div>
        ...
        <h2 style="font-size: large;">${selectedDoc.idDocumento}</h2> 
        </div>
        <div style="display: flex; justify-content: space-between;">

        <div>
         <p style="margin-top: 50px; text-align: center; border-top: solid;width: 220px; margin-right: 8px;">${selectedDoc.responsavel.destinatario}</p>
        <p style="text-align: center; margin-top: 20px;">Data:____/____/_____</p>
        </div>
       
         <div>
         <p style="margin-top: 50px; text-align: center; border-top: solid;width: 220px; margin-right: 8px;">${selectedDoc.responsavel.transporte}</p>
        <p style="text-align: center; margin-top: 20px;">Data:____/____/_____</p>
        </div>
       
        <div>
        <p style="margin-top: 50px; text-align: center; border-top: solid;width: 220px;">${selectedDoc.responsavel.remetente}</p>
        <p style="text-align: center; margin-top: 20px;">Data:____/____/_____</p>
        </div>
        </div>
        </div>
        ...
        <div style="display: flex; justify-content: space-between;">
          ...
          <div>
         <p style="margin-top: 50px; text-align: center; border-top: solid;width: 220px; margin-right: 8px;">${selectedDoc.responsavel.remetente}</p>
           <p style="text-align: center; margin-top: 20px;">Data:____/____/_____</p>
          </div>
          <div>
          <p style="margin-top: 50px; text-align: center; border-top: solid;width: 220px;">${selectedDoc.responsavel.destinatario}</p>
          <p style="text-align: center; margin-top: 20px;">Data:____/____/_____</p>
          </div>
         </div>
        </div>
  ...
  const fetchRhDocs = async () => {
    try {
      const docRef = doc(db, 'gerenciaMK', 'MKDocs'); // Referência ao documento RhDocs
      ...
      if (docSnap.exists()) {
        ...
        const filteredDocs = currentUser.cargo === 'Marketing'
    ...
    <div className='pt-20 bg-altBlue'>
      ...
      {docs.length > 0 ? (
        <ul className='p-8 grid lg:grid-cols-2 gap-3'>
          {docs.map((doc) => (
            <li key={doc.id} className="mb-4 p-4 flex justify-between gap-2 border rounded bg-white shadow-xl">
              <div>
...
export default ListMarketingDocs;
