...
const MkNewTicketModal = ({ isOpen, onClose, addTicket }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentUser.cargo === 'Supervisor' || currentUser.cargo === 'Marketing' || currentUser.cargo === 'Claudemir') {
    ...
    try {
      const orderControlRef = doc(db, 'ordersControl', 'MkOrders'); // Referência ao documento de controle de pedidos
      ...
      if (orderControlSnap.exists()) {
        ...
        if (/^MK[A-Z]\d{3}$/.test(lastOrder)) {
          ...
          nextOrder = 'MK' + nextOrderLetter + nextOrderNumber.toString().padStart(3, '0'); // Gera o próximo número de pedido
      ...
      const newTicket = {
        cidade: (currentUser.cargo === 'Supervisor' || currentUser.cargo === 'Marketing' || currentUser.cargo === 'Claudemir') ? selectedCity : userDetails.cidade,
        ...
        loja: (currentUser.cargo === 'Supervisor' || currentUser.cargo === 'Marketing' || currentUser.cargo === 'Claudemir') ? selectedStore : userDetails.loja,
      ...
      const newTicketRef = doc(db, 'marketingChamados', 'aberto', 'tickets', nextOrder); // Referência ao novo chamado no Firestore
      ...
      // Coleta os tokens dos usuários com cargo "Marketing"
      ...
      citiesSnapshot.forEach((cityDoc) => {
        ...
        Object.keys(cityData).forEach((userKey) => {
          ...
          if (user.cargo === 'Marketing' && Array.isArray(user.token)) { // Certifique-se de que `token` é um array
      ...
      const notification = {
        title: nextOrder,
        body: description,
        click_action: "https://drogalira.com.br/mkchamados",
    ...
    <div className="fixed inset-0 z-40 pt-1 bg-black bg-opacity-50 flex items-center justify-center">
      <MyModal isOpen={isOpen} onClose={onClose} showCloseButton={false}>
        ...
        <form onSubmit={handleSubmit}>
          {(currentUser.cargo === 'Supervisor' || currentUser.cargo === 'Marketing' || currentUser.cargo === 'Claudemir') && (
...
export default MkNewTicketModal;
