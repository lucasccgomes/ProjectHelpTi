...
const GerenciaRh = () => {
  ...
  const fetchLastDocumentNumber = async () => {
    try {
      const docRef = doc(db, 'gerenciaRh', 'controlDoc'); // Referência ao documento controlDoc
  ...
  const generateNextDocNumber = (lastDocNumber) => {
    ...
    return `RHD${letter}${number.toString().padStart(3, '0')}`;
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const documentRef = doc(db, 'gerenciaRh', 'RhDocs');
