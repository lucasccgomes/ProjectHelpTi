...
import MkNewTicketModal from '../components/MkNewTicketModal/MkNewTicketModal';
...
const MkUserTickets = () => {
  ...
  const handleAuthorize = async (ticketId, ticketOrder) => {
    try {
      ...
      // Busca os tokens dos usuários com o cargo "Marketing"
      const usersSnapshot = await getDocs(
        query(collection(db, 'usuarios'), where('cargo', '==', 'Marketing'))
      );
      ...
      usersSnapshot.forEach((doc) => {
        ...
        if (userData.token) {
          tiTokens.push(userData.token); // Armazena os tokens dos usuários "Marketing"
        }
      });

      // Dispara a notificação para os usuários "Marketing"
  ...
  const handleDeny = async () => {
    ...
    try {
      const ticketDocRef = doc(db, 'marketingChamados', 'aberto', 'tickets', ticketToDeny.id);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const ticketsSnapshot = await getDocs(collection(db, 'marketingChamados', 'aberto', 'tickets'));
  ...
  useEffect(() => {
    const fetchTickets = () => {
      let abertoRef = collection(db, 'marketingChamados', 'aberto', 'tickets');
      let q;
    ...
    <div className="bg-altBlue justify-center items-center">
      ...
      <div className='w-full bg-altBlue p-4 fixed mt-[3.5rem] z-10'>
        {currentUser.cargo !== 'Supervisor' && (
          <div className='w-full flex'>
            <div className='flex items-center px-1 text-white rounded-b-xl bg-primaryBlueDark -mt-4 lg:hidden'>
              <animated.button
                style={toggleSpring}
                className={`flex items-center ${showStoreTickets ? '' : ''} hover:bg-${showStoreTickets ? '' : ''} text-white font-bold ml-4`}
        ...
        <div className='flex flex-col lg:flex-row lg:gap-4 w-full items-center mb-4 mt-4 lg:mt-0'>
          ...
          <h2 className="text-2xl w-full font-bold text-center lg:text-end text-white">Marketing Chamados</h2>
          ...
          <MkNewTicketModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} addTicket={addTicket} />
        </div>
